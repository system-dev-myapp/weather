fetch(
    ...
    .then(function (responseT) {
        ...
        async function handleCallApiCodeWeather() {
            ...
            const found = listCode.find(
                (element) => element.key === wC.condition.code
            );
      ...
      `;
        ...
        if (h.day.avgtemp_f >= 70) {
            ...
            Temp.forEach(function (ele, index) {
                if (
                    listTemp[index - 1] >= h.day.mintemp_f &&
                    listTemp[index - 1] <= h.day.avgtemp_f - 1
                ) {
                    ...
                    listTemp[index - 1] >= h.day.avgtemp_f + 1 &&
                    listTemp[index - 1] < h.day.maxtemp_f
                ) {
        ...
        if (h.day.maxwind_mph <= 10) {
            ...
            Wind.forEach(function (ele, index) {
                if (listWind[index - 1] >= 0 && listWind[index - 1] <= 5) {
                    ...
                    listWind[index - 1] > 5 &&
                    listWind[index - 1] <= h.day.maxwind_mph
                ) {
